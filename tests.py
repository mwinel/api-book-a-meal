...
class TestBase(TestCase):
	...
	def test_delete_a_menu(self):
		...
		response = self.app.post("/api/v1/menus", data = self.menu)
		self.assertEqual(response.status_code, 201)
		response = self.app.delete("/api/v1/menus/1", data = self.menu)
		self.assertEqual(response.status_code, 200)
		response = self.app.get("/api/v1/menus/1", data = self.menu)
		output = json.loads(response.data)
		self.assertEqual(output, {'message': 'Menu not found'})
